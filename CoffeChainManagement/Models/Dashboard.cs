...
namespace CoffeChainManagement.Models
{
	...
	public class Dashboard : DbConnection_
	{
		//Fields & Properties
			...
			using (var connection = GetConnection())
			{
				...
				using (var command = new SqlCommand())
				{
					...
					command.CommandText = @"SELECT ProductName, Stock
                        FROM Product
                        WHERE Stock <= 10 AND IsDiscontinued = 0";

					reader = command.ExecuteReader();

					while (reader.Read())
					{
						string productName = reader.IsDBNull(0) ? "Unknown" : reader.GetString(0); // Kiểm tra null cho ProductName
						int stockValue = reader.IsDBNull(1) ? 0 : reader.GetInt32(1);               // Kiểm tra null và cast an toàn cho Stock

						UnderstockList.Add(new KeyValuePair<string, int>(productName, stockValue));
					}
